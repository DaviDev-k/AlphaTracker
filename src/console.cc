...
void drawBox(const char *title, int x, int y, int w, int h, int align, const char *color, int style) {
    int len = w - strlen(title) - 4;
    int left, right;
...
char *tab(int len, int num) {
    char *tab = new char[len - num + 1];
